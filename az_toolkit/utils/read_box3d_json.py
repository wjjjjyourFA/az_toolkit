...
def read_box3d_json(file_path):
    ...
    box3d_list = []  # 初始化一个空列表用于存放三维边界框
    label_list = []  # 初始化一个空列表用于存放每个物体的类别
    ...
    for i in range(len(label_info)):
        ...
        box3d = [position['x'], position['y'], position['z'], scale['x'], scale['y'], scale['z'], rotation['z']]

        # 组装一个三维边界框 [y, -x, z, scale_y, scale_x, scale_z, rotation_z]
        # box3d = [position['y'], -position['x'], position['z'], scale['y'], scale['x'], scale['z'], rotation['z']]

        # 将当前物体的边界框和类别信息添加到对应的列表中
        box3d_list.append(box3d)
        label_list.append(obj_type)

    # 将列表转换为 NumPy 数组
    box3d_np = np.array(box3d_list)

    return box3d_np, label_list  # 返回三维边界框和标签
