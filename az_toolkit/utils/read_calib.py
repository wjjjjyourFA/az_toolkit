    ...
    with open(calib_path, "r") as f:
        for line in f:
            ...
            if line == "K":
                ...
                #     list(map(float, f.readline().strip().split()))
                # ], dtype=np.float64)
                calib_data["K"] = np.array([list(map(float, f.readline().strip().split()))
                                            for _ in range(3)], dtype=np.float64)
            elif line == "k1k2k3":
                # 读取径向畸变系数
                calib_data["k1k2k3"] = np.array(list(map(float, f.readline().strip().split())), dtype=np.float64)
            elif line == "p1p2":
                # 读取切向畸变系数
                calib_data["p1p2"] = np.array(list(map(float, f.readline().strip().split())), dtype=np.float64)
            elif line == "Rotate":
                # 读取旋转矩阵
                calib_data["Rotate"] = np.array([list(map(float, f.readline().strip().split()))
                                                 for _ in range(3)], dtype=np.float64)
            elif line == "Translate":
                # 读取平移向量
                calib_data["Translate"] = np.array(list(map(float, f.readline().strip().split())), dtype=np.float64)
            elif line == "P":
                # 读取投影矩阵
                projection_matrix = []
                for _ in range(3):
                    projection_matrix.append(list(map(float, f.readline().strip().split())))
                calib_data["P"] = np.array(projection_matrix, dtype=np.float64)
...
def read_base_calib(calib_path):
    calib_data = {
        "Rotate": None,
        "Translate": None,
        "RT": None
...
if __name__ == "__main__":
    calib_file = GLOBAL_CONFIG["simple_path"] + "/samples_common/calib/KK.ini"
