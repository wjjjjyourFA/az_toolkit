...
try:
    from ..common.misc import atomic_write_lines
except ImportError:
    from az_toolkit.common.misc import atomic_write_lines


def extract_timestamp(root_path, data_subfolder, file_extension, output_file, strip_prefix=False):
    """
    ...
    print(f"✅ Data path confirmed: {data_path}")
...
def extract_timestamp_fixed(root_path, data_subfolder, file_extension, output_file_path, strip_prefix=False):
    # ts_file = output_file_path + f"{data_subfolder}" + f"_timestamp.txt"
    ts_file = os.path.join(output_file_path, f"{data_subfolder}_timestamp.txt")
    extract_timestamp(root_path, data_subfolder, file_extension, ts_file, strip_prefix)
...
def write_timestamp(ts_file, timestamp_list):
    ...
    atomic_write_lines(ts_file, (str(timestamp) for timestamp in timestamp_list))
...
def remove_unmatched_timestamp(base_timestamp, base_delete_list):
