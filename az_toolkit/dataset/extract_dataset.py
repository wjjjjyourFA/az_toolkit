...
try:
    ...
    from .tools.batch_handler import *
    from .config import GLOBAL_CONFIG
except ImportError:
    # 作为独立脚本运行
    from az_toolkit.common.misc import *
    from az_toolkit.tools.batch_handler import *
...
class Extractor:
    ...
    def run(self):
        ...
        self.extract_files_interval(image_timestamp_list, self.prefix, "lidar", ".bin", self.PrefixSave,
                                    self.time_interval)
        self.extract_files_interval(image_timestamp_list, self.prefix, "image", ".jpg", self.PrefixSave,
                                    self.time_interval)
        self.extract_files_interval(image_timestamp_list, self.prefix, "infra", ".jpg", self.PrefixSave,
                                    self.time_interval)
        self.extract_files_interval(image_timestamp_list, self.prefix, "star", ".jpg", self.PrefixSave,
                                    self.time_interval)
        self.extract_files_interval(image_timestamp_list, self.prefix, "radar", ".txt", self.PrefixSave,
                                    self.time_interval)

    def extract_files_interval(self, timestamp_list, source_dirs, sensor_name, file_extension, target_dirs,
                               time_interval):
...
if __name__ == '__main__':
    ...
    ''' 单数据文件处理 '''
    # args.data_root = "./../../../test_data/samples_common"
    # args.target_folder = "./../../../test_data/samples_dataset"
    # extractor = Extractor(args=args)
    # extractor.run()

    """ auto for data root folder
    批量处理上述格式的数据文件 """
    root_paths = GLOBAL_CONFIG["simple_path"]
    batch_handler(root_paths, target_suffix="_extracted", args=args, process_class=Extractor)
