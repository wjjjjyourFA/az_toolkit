...
from az_toolkit.az_math.transforms.quat2rotation import *
from az_toolkit.utils.read_calib import *
...
def compose_projection_matrix(R, T, K, verbose=True):
    """
    从 R, T, K 计算投影矩阵 P
...
def main_all(calib_file_test="", calib_file_gt=""):
    # Parse the AutoCalib_PSO.txt file
    auto_calib_data = read_pso_calib_all(calib_file_test)
...
if __name__ == "__main__":
    ...
    # calib_file_gt = test_path + "/testdata/AutoCalib_PSO_gt3.txt"
    # main(calib_file_test, calib_file_gt)

    """ 计算 多组P的 平均值的 欧拉角 平移 误差 """
    # calib_file_test = test_path + "/testdata/auto_all.txt"
    # calib_file_gt = test_path + "/testdata/manual.ini"
    # main_all(calib_file_test, calib_file_gt)
    ...
    """ """
    # R = [
    #   [0.0191, -0.9776, -0.0150],
    #   [0.0621, 0.0165, -0.9867],
    #   [0.9993, 0.0259, 0.0263]
    # ]
    # T = [-94.1483, -35.6569, -332.2830]
    ...
    #   [0.99811121,  0.01392848,  0.0598332 ]
    # ]
    # T = [-41.6067333, -71.73449723, 30.5194]
    # compose_projection_matrix(R, T, K)

    """ """
    P = [
      [1007, -1943.23, 95.6702, -51418.9],
      [577.403, -28.8039, -1925.1, -126289],
      [0.998111, 0.0139286, 0.059833, 30.5194]
    ]
    intrinsic_matrix = K
    R, T = decompose_projection_matrix(P, intrinsic_matrix)
    print("\nR ():")
