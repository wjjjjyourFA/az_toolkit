...
if __name__ == '__main__':
    ...
    for i in range(len(list)):
        filesroot = os.path.join("/home/feikexin/fkx/JCJQ/Dataset/", list[i])
        fileroot = os.listdir(filesroot)
        for j in range(len(fileroot)):
            dataroot = os.path.join(filesroot, fileroot[j])
            lidar_path = os.path.join(dataroot, "LIDAR/")
            image_path = os.path.join(dataroot, "IMAGE/")
            imageBox_path = os.path.join(dataroot, "IMGBox/")
            infra_path = os.path.join(dataroot, "INFRA/")
            radar_path = os.path.join(dataroot, "Radar/")
            lidarbox_path = os.path.join(dataroot, "LIDARBox/")
            ...
            for i in range(0, len(filelist)):
                tmpname = filelist[i]
                if tmpname.split('.')[-1] == 'jpg' or tmpname.split('.')[-1] == 'jpeg' or tmpname.split('.')[
                    -1] == 'png':
            ...
            for fileidx in range(0, len(Timestamplist)):
                ...
                if os.path.exists(radarname):
                    radar_data = np.loadtxt(radarname, ndmin=2)
                ...
                if os.path.exists(radarname):
                    ...
                    if radarPOSE_EXIST:
                        ...
                        tmp = np.pad(tmp.transpose(1, 0), ([0, 1], [0, 0]), constant_values=1.0,
                                     mode="constant").transpose(1,
                                                                0)
